...
class Enemy(Element):
	...
	def heuristic(self, x, y, player_x, player_y):
		return math.sqrt((x - player_x) ** 2 + (y - player_y) ** 2)
	
	def nextNode(self, dirs = ["UP", "DOWN", "LEFT", "RIGHT"]):
		player_x, player_y = P1.getNodePos()
		possible_moves = []

		if self.isSafeNode(self.lastNode[0], self.lastNode[1] - 1):  
			possible_moves.append((self.lastNode[0], self.lastNode[1] - 1, "UP"))
		if self.isSafeNode(self.lastNode[0], self.lastNode[1] + 1):  
			possible_moves.append((self.lastNode[0], self.lastNode[1] + 1, "DOWN"))
		if self.isSafeNode(self.lastNode[0] - 1, self.lastNode[1]):  
			possible_moves.append((self.lastNode[0] - 1, self.lastNode[1], "LEFT"))
		if self.isSafeNode(self.lastNode[0] + 1, self.lastNode[1]):  
			possible_moves.append((self.lastNode[0] + 1, self.lastNode[1], "RIGHT"))

		if len(possible_moves) == 0:
			return None

		best_move = min(possible_moves, key=lambda move: self.heuristic(move[0], move[1], player_x, player_y))
