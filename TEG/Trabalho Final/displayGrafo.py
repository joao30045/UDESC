...
class Grafo:
    ...
    def id_to_random_color(self, number):
        if not number in self.colors:
            r = random.random()
            g = random.random()
            b = random.random()
            self.colors[number] = (r, g, b, 1.0)
        return self.colors[number]

    def addAresta(self, xa, ya, xb, yb, vertice, vizinho, grupo) -> object:
        self.grupos[vertice] = grupo
        self.grupos[vizinho] = grupo
        self.coordenadas[vertice] = (xa, ya)
        self.coordenadas[vizinho] = (xb, yb)
    ...
    def showMapa3D(self) -> None:
        ...
        random.SystemRandom()
        grafo_3D = {
            v: [self.coordenadas[v][0], self.coordenadas[v][1], self.grupos[v]] for v in self.getVertices()
        }
        ...
        trace_vertices = go.Scatter3d(
            ...
            marker=dict(symbol='circle',
                        size=10,
                        color=[self.id_to_random_color(self.grupos[v]) for v in self.getVertices()])
        )
...
def GrafoFromFile(enderecoArquivo = "", dirigido = False) -> Grafo:
    ...
    split = lambda string: [*map(float, string.split(" "))]

    with open(enderecoArquivo, "r") as arquivo:
        for linha in arquivo.readlines():
            aresta = split(linha)
            if len(aresta) > 1:
                xa, ya, xb, yb, a, b, c = aresta
                grafo.addAresta(xa, ya, xb, yb, a, b, c)
...
if __name__ == "__main__":
    name= './arestasGrafo.csv'#grafo.csv'#
